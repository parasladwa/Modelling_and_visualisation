...
def main():
    ...
    parser.add_argument("-m", "--method", type = str, default = "gs", help = "choose method from gs or j or gsonly, (gauss siedel(SOR) or jacobi or gauss seidel only)")
    ...
    if args.method == 'gsonly' and args.log:
        outfile_gsonly = open('magnet_gsonly.txt', 'w')
        outfile_gsonly.write(f"<i> <j> <vector_potential> <magnetic_field>\n")
        
        for i in range(len(A[args.N//2])):
            for j in range(len(A[args.N//2, i])):
                for k in range(len(A[args.N//2, i])):
                    outfile_gsonly.write(f"{i} {j} {k} {A[i, j, k]} {Bx[i, j, k]} {By[i, j, k]} {Bz[i, j, k]}\n")
